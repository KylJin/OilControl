...
class Refinery(object):
    ...
    def get_info(self, action):
        vertex_name = f'refinery{self.key}'

        # 获取节点信息
        vertex_info = {
            'name': vertex_name,
            'value': {
                '原油种类': self.crude.crude_kinds,
                '原油加工方案': action['scheme_idx'],
                '原油加工量': action['process'],
                '原油库存': list(self.crude.storage.values()),
                '汽油库存': self.gas.storage,
                '柴油库存': self.diesel.storage
            }
        }
        ...
        for nbr, delivery in zip(gas_connections, action['gas_delivery']):
            ...
            for i, road in enumerate(roads):
                ...
                if '汽油' in trans_collection[route]:
                    trans_collection[route]['汽油'] += delivery[i]
                else:
                    trans_collection[route]['汽油'] = delivery[i]
        ...
        for nbr, delivery in zip(diesel_connections, action['diesel_delivery']):
            ...
            for i, road in enumerate(roads):
                ...
                trans_collection = trans_info[str(road.way)]
                route = (vertex_name, nbr_name)
                if '柴油' in trans_collection[route]:
                    trans_collection[route]['柴油'] += delivery[i]
                else:
                    trans_collection[route]['柴油'] = delivery[i]
