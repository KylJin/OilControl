...
class MY_Geo(MY_GeoChartBase):
    ...
    def _feed_data(self, data_pair: types.Sequence, type_: str) -> types.Sequence:
        result = []
        if len(data_pair[0]) == 2:
            for n, v in data_pair:
                try:
                    if type_ == ChartType.LINES:
                        a, b = n
                        f, t = self.get_coordinate(a), self.get_coordinate(b)
                        result.append({"name": "{}->{}".format(a, b), "coords": [f, t], "inf": v})
                    else:
        ...
        elif len(data_pair[0]) == 3:
            for n, v, d in data_pair:
                try:
                    if type_ == ChartType.LINES:
                        a, b = n
                        f, t = self.get_coordinate(a), self.get_coordinate(b)
                        ...
                        lng, lat = self.get_coordinate(n)
                        result.append({"name": n, "value": [lng, lat, v], "inf": d})
                except TypeError as err:
                    if self._is_ignore_nonexistent_coord is not True:
                        raise NonexistentCoordinatesException(err, (n, v))
        return result
